...
function buildSigHtml(f: typeof SIG_FIELDS, logoUrl: string) {
  ...
  <tr>
    ...
    <td style="padding:12px 18px;vertical-align:middle;border-top:1px solid #e8e8e8;border-bottom:1px solid #e8e8e8;border-right:1px solid #e8e8e8;">
      <p style="margin:0 0 2px 0;font-family:Arial,sans-serif;font-size: 16px;font-weight:700;color:#1a1a1a;letter-spacing:0.03em;">${f.nombre}</p>
      <p style="margin:0 0 10px 0;font-family:Arial,sans-serif;font-size: 12px;letter-spacing:0.2em;text-transform:uppercase;color:#C8A84B;">${f.cargo}</p>
      <table cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td style="padding:2px 0;font-family:Arial,sans-serif;font-size: 13px;color:#888;padding-right:6px;">📞</td>
          <td style="padding:2px 0;font-family:Arial,sans-serif;font-size: 13px;color:#555;">${f.telefono}</td>
        </tr>
        <tr>
          <td style="padding:2px 0;font-family:Arial,sans-serif;font-size: 13px;color:#888;padding-right:6px;">✉</td>
          <td style="padding:2px 0;font-family:Arial,sans-serif;font-size: 13px;color:#555;"><a href="mailto:${f.email.replace(/[\[\]]/g,'')}" style="color:#9B6920;text-decoration:none;">${f.email}</a></td>
        </tr>
        <tr>
          <td style="padding:2px 0;font-family:Arial,sans-serif;font-size: 13px;color:#888;padding-right:6px;">🌐</td>
          <td style="padding:2px 0;font-family:Arial,sans-serif;font-size: 13px;"><a href="https://${f.web}" style="color:#9B6920;text-decoration:none;">${f.web}</a></td>
        </tr>
      </table>
      <p style="margin:10px 0 0 0;font-family:Arial,sans-serif;font-size: 11px;letter-spacing:0.18em;text-transform:uppercase;color:#bbb;">Climatización · Refrigeración · Todo Chile</p>
    </td>
  </tr>
</table>`
}
...
export default function FirmaEmailPage() {
  ...
  const inp: React.CSSProperties = {
    ...
    fontFamily: 'var(--font-outfit), sans-serif', fontSize: 14, fontWeight: 300,
  ...
  return (
    <div className="fe-root">
      ...
      <div className="fe-header">
        <div>
          <p style={{ fontFamily: 'var(--font-josefin), sans-serif', fontSize: 9.5, letterSpacing: '0.36em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 6 }}>
            Identidad Corporativa
          </p>
          <h1 style={{ fontFamily: 'var(--font-josefin), sans-serif', fontSize: 24, fontWeight: 300, letterSpacing: '-0.01em' }}>
            Firma de Email
          </h1>
        </div>
        <button
          ...
          <tbody>
            <tr>
              ...
              <td style={{ padding: '12px 18px', verticalAlign: 'middle', border: '1px solid rgba(255,255,255,.08)', borderLeft: 'none', background: '#151515' }}>
                <p style={{ margin: '0 0 2px', fontFamily: 'Arial,sans-serif', fontSize: 16, fontWeight: 700, color: '#f0eeeb', letterSpacing: '0.03em' }}>{fields.nombre}</p>
                <p style={{ margin: '0 0 10px', fontFamily: 'Arial,sans-serif', fontSize: 12, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C8A84B' }}>{fields.cargo}</p>
                <table cellPadding="0" cellSpacing="0">
                  <tbody>
                    <tr><td style={{ padding: '2px 6px 2px 0', fontFamily: 'Arial,sans-serif', fontSize: 13, color: '#666' }}>📞</td><td style={{ padding: '2px 0', fontFamily: 'Arial,sans-serif', fontSize: 13, color: '#999' }}>{fields.telefono}</td></tr>
                    <tr><td style={{ padding: '2px 6px 2px 0', fontFamily: 'Arial,sans-serif', fontSize: 13, color: '#666' }}>✉</td><td style={{ padding: '2px 0' }}><a href={`mailto:${fields.email}`} style={{ color: '#C8A84B', textDecoration: 'none', fontFamily: 'Arial,sans-serif', fontSize: 13 }}>{fields.email}</a></td></tr>
                    <tr><td style={{ padding: '2px 6px 2px 0', fontFamily: 'Arial,sans-serif', fontSize: 13, color: '#666' }}>🌐</td><td style={{ padding: '2px 0' }}><a href={`https://${fields.web}`} style={{ color: '#C8A84B', textDecoration: 'none', fontFamily: 'Arial,sans-serif', fontSize: 13 }}>{fields.web}</a></td></tr>
                  </tbody>
                </table>
                <p style={{ margin: '10px 0 0', fontFamily: 'Arial,sans-serif', fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#444' }}>Climatización · Refrigeración · Todo Chile</p>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div style={{ border: '1px solid var(--border)', marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px', borderBottom: '1px solid var(--border)' }}>
          <span style={{ fontFamily: 'var(--font-josefin), sans-serif', fontSize: 10.5, letterSpacing: '0.24em', textTransform: 'uppercase', color: 'var(--dim)' }}>
        ...
        <div style={{ padding: 18, overflowX: 'auto' }}>
          <pre style={{ fontFamily: 'Courier New, monospace', fontSize: 13, color: 'rgba(255,255,255,0.4)', whiteSpace: 'pre-wrap', wordBreak: 'break-all', lineHeight: 1.6 }}>
            {sigHtml}
          </pre>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(200,168,75,.06)', border: '1px solid rgba(200,168,75,.15)', padding: '16px 20px', marginBottom: 40 }}>
        <p style={{ fontFamily: 'var(--font-josefin), sans-serif', fontSize: 10.5, letterSpacing: '0.28em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 12 }}>
        ...
        <ol style={{ paddingLeft: 18, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            ...
            <li key={title} style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
