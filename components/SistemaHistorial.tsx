...
import { SistemaRecord, SistemaTipo, SISTEMA_LABELS, makeId } from '@/lib/sistemas-store'
import { apiFetchSistemaHistory, apiCreateSistemaRecord, apiDeleteSistemaRecord } from '@/lib/sistemas-api'
...
export default function SistemaHistorial({ tipo }: { tipo: SistemaTipo }) {
  const router = useRouter()
  const label = SISTEMA_LABELS[tipo]
  const basePath = `/admin/informe-${tipo}`

  const [records, setRecords] = useState<SistemaRecord[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  async function load() {
    try { setRecords(await apiFetchSistemaHistory(tipo)) }
    catch { /* silently fail */ }
    finally { setLoading(false) }
  }

  useEffect(() => { void load() }, [tipo]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleDelete(id: string, codigo: string) {
    ...
    await apiDeleteSistemaRecord(id)
    setRecords(r => r.filter(x => x.id !== id))
  }

  async function handleNuevaVersion(record: SistemaRecord) {
    ...
    const newId = makeId()
    await apiCreateSistemaRecord(newId, tipo, { ...record.data, meta: { ...record.data.meta, codigo: newCodigo } }, 'borrador', record.id)
    router.push(`${basePath}/nuevo?id=${newId}`)
  }
    ...
    <>
      ...
      <div className="it-wrap">
        <div>
          ...
          <h1 style={{ fontFamily: 'Josefin Sans, sans-serif', fontSize: 22, fontWeight: 200, letterSpacing: '0.06em' }}>{label.es}s</h1>
        </div>

        <div className="it-nueva">
          <div className="it-nueva-text">
            <h2>Nuevo {label.es}</h2>
            <p>Mediciones de ciclo por unidad/circuito, fotos, diagrama y cálculo automático de superheat/subcool</p>
          </div>
          <Link href={`${basePath}/nuevo`} className="btn-primary">
            + Crear informe
          </Link>
        </div>
                  ...
                  <tr key={r.id}>
                    ...
                    <td>
                      <div className="admin-doc-actions">
                        <Link href={`${basePath}/nuevo?id=${r.id}`} className="doc-btn">
                          <EditIcon /> Editar
                        </Link>
                        <Link href={`${basePath}/nuevo?id=${r.id}&print=1`} className="doc-btn doc-btn-accent">
                          <PdfIcon /> PDF
                        </Link>
